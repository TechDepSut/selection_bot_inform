...
async def moder(message: Message):
    await message.answer("Отдел Модерации работает с организаторами проекта 1NFORM. Модераторы выстраивают позиционирование нашей команды: следят за соблюдением правил и регламента, а также модерируют контент в социальных сетях проекта. Перед началом активной фазы сотрудники отдела проводят доскональную проверку всех социальных сетей каждого организатора проекта.\n\nПодать заявку в отдел можно тут:\nhttps://forms.gle/mxrAfhdd2pHByFhJA")
    await anythingElse(message.peer_id)
...
async def media(message: Message):
    await message.answer("Снимают фото и видео, придумывают идеи и пишут раскадровки, обрабатывают и анимируют, все это делает медиа-отдел для социальных сетей проекта и его внешнего представления на других площадках. Увидеть работы можно в группах 1NFORM, Абитуриенты и НеНочная.\n\nПодать заявку в отдел можно тут:\nhttps://docs.google.com/forms/d/e/1FAIpQLSe8nerUPgnN3yvFIAIioaUvmPzjTyezU3lxxAQvKYDPjRChFw/viewform?usp=sf_link")
    await anythingElse(message.peer_id)
